    ...
    <div className="animate-fadeIn max-w-6xl mx-auto pb-20">
      <div className={`p-8 md:p-10 rounded-[40px] mb-12 flex flex-col md:flex-row justify-between items-center gap-6 transition-all ${darkMode ? 'glass' : 'bg-white border border-slate-100 shadow-xl shadow-slate-200/50'}`}>
        <div className="text-center md:text-right">
          ...
          <p className={`font-bold text-sm mt-1 ${darkMode ? 'text-slate-500' : 'text-slate-400'}`}>إدارة المحتوى والأمان لـ {settings.siteName}</p>
        </div>
      ...
      {activeTab === 'ads' && (
        <div className={`p-10 rounded-[50px] space-y-8 animate-fadeIn transition-all max-w-2xl mx-auto ${darkMode ? 'glass' : 'bg-white border border-slate-100 shadow-xl'}`}>
          <div className="text-center mb-10">
            ...
            <h3 className={`text-3xl font-black mb-4 ${darkMode ? 'text-white' : 'text-slate-900'}`}>إعدادات AdSense والموقع</h3>
          </div>
          <div className="space-y-6">
            <input 
      ...
      {activeTab === 'security' && (
        <div className={`p-10 rounded-[50px] space-y-8 animate-fadeIn transition-all max-w-2xl mx-auto ${darkMode ? 'glass' : 'bg-white border border-slate-100 shadow-xl'}`}>
          <div className="text-center mb-10">
            <span className="text-6xl mb-6 block">🔐</span>
            <h3 className={`text-3xl font-black mb-4 ${darkMode ? 'text-white' : 'text-slate-900'}`}>تغيير كلمة المرور</h3>
            <p className="opacity-60 font-bold">قم بتحديث رمز الدخول للوحة التحكم لضمان الحماية.</p>
          </div>
          <div className="space-y-6">
