...
const Home: React.FC<HomeProps> = ({ posts, onPostClick, darkMode = true }) => {
  ...
  return (
    <div className="space-y-16 animate-fadeIn" dir="rtl">
      ...
      <section className="relative group cursor-pointer" onClick={() => onPostClick(trendingPost)}>
        <div className="relative h-[450px] md:h-[600px] rounded-[40px] md:rounded-[60px] overflow-hidden shadow-2xl border border-white/5">
          ...
          <div className="absolute bottom-10 right-8 left-8 md:bottom-16 md:right-16 md:left-16 text-white">
            <span className="bg-orange-600 px-5 py-2 rounded-xl text-xs font-black uppercase mb-6 inline-block shadow-xl animate-pulse tracking-widest">همزة اليوم 🔥</span>
      ...
      {/* قسم الكوبونات - لزيادة التحويل (Conversion) */}
      <div className="bg-emerald-500/10 border border-emerald-500/20 p-8 rounded-[40px] flex items-center justify-between flex-wrap gap-6 relative overflow-hidden">
        <div className="absolute top-0 left-0 w-32 h-32 bg-emerald-500/10 rounded-full -translate-x-16 -translate-y-16"></div>
        <div className="flex items-center gap-4 relative z-10">
          ...
          <div>
            <h3 className="text-2xl font-black text-emerald-500">كوبونات تيمو (Temu) المغرب</h3>
            <p className="text-sm font-bold opacity-60">انسخ الكود واحصل على خصم 90% وشحن مجاني</p>
          </div>
        </div>
        <div className="flex gap-4 flex-wrap relative z-10">
          {['epm88', 'maroc99'].map(code => (
            <div 
              key={code} 
              className="bg-black/40 px-8 py-4 rounded-2xl border-2 border-dashed border-emerald-500/40 font-mono font-black text-emerald-500 text-xl shadow-sm hover:scale-105 transition-transform cursor-pointer"
              onClick={() => {navigator.clipboard.writeText(code); alert('تم نسخ كود الخصم: ' + code)}}
            >
              {code}
            </div>
          ))}
      ...
      <div className="space-y-10">
        <h2 className="text-3xl font-black flex items-center gap-4">
          <span className="w-3 h-10 bg-emerald-500 rounded-full"></span> أحدث مراجعات وعروض الأفلييت
        </h2>
          ...
          {otherPosts.map(post => (
            <div key={post.id} className="group cursor-pointer bg-white/5 rounded-[45px] overflow-hidden border border-white/5 hover:border-emerald-500/30 hover:shadow-2xl transition-all duration-500" onClick={() => onPostClick(post)}>
              <div className="relative h-64 overflow-hidden">
                ...
                <div className="absolute top-6 left-6 bg-black/70 backdrop-blur-md px-4 py-2 rounded-xl text-[10px] font-black text-white uppercase border border-white/10">
                  {post.category}
                </div>
              </div>
              <div className="p-8">
                ...
                <div className="flex items-center justify-between mt-auto">
                  <span className="text-emerald-500 font-black text-sm">التفاصيل الكاملة ←</span>
