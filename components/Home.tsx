          ...
          <section className={`relative overflow-hidden mb-20 rounded-[48px] p-12 md:p-24 transition-all duration-500 ${darkMode ? 'bg-slate-900 border border-slate-800' : 'bg-slate-900 shadow-2xl shadow-emerald-900/10'}`}>
            ...
            <div className="relative z-10 max-w-4xl text-right">
              ...
              <p className="text-slate-400 text-xl md:text-2xl mb-12 leading-loose max-w-3xl font-light">
                نقدم لك محتوى مدروساً بعناية يغطي مجالات التقنية، ريادة الأعمال، ومراجعات المنتجات، لنصحبك في رحلة نحو التميز الرقمي.
              </p>
              
              <div className="flex flex-wrap gap-4 items-center">
                {/* Featured Reviews Button */}
                <button 
                  onClick={() => onCategoryClick(Category.REVIEWS)}
                  className="bg-emerald-600 hover:bg-emerald-500 text-white px-10 py-5 rounded-2xl font-black text-xl transition-all shadow-xl shadow-emerald-900/40 flex items-center gap-4 group active:scale-95"
                >
                ...
                <div className="flex flex-wrap gap-3">
                  {Object.values(Category).filter(c => c !== Category.REVIEWS).slice(0, 3).map(cat => (
                    <button 
                      key={cat}
                      onClick={() => onCategoryClick(cat)}
                      className="bg-white/5 hover:bg-white/10 text-white px-6 py-4 rounded-2xl font-bold transition-all border border-white/10 backdrop-blur-md"
                    >
                      {cat}
                    </button>
                  ))}
