...
const ArticleDetail: React.FC<ArticleDetailProps> = ({ article, onBack, siteName, adsenseCode, relatedArticles, onArticleClick, onLike, onAddComment, darkMode }) => {
  ...
  // توليد الرابط النهائي للمشاركة مع معالجة المسافات
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: article.name,
          text: `شاهد هذا المقال الرائع من ${siteName}:\n${article.name}`,
          url: shareUrl,
        });
      } catch (err) {
  ...
  const shareOnWhatsApp = () => {
    const message = `شاهد هذا المقال الرائع من ${siteName}:\n${article.name}\n\nالرابط:\n${shareUrl}`;
  ...
  return (
    <article className="max-w-4xl mx-auto pb-20 animate-fadeIn relative text-right" dir="rtl">
      <div className="fixed top-0 right-0 h-1.5 bg-emerald-500 z-[100] transition-all duration-100" style={{ width: `${scrollProgress}%` }}></div>
      ...
      <div className={`rounded-[60px] shadow-2xl overflow-hidden border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-white'}`}>
        <header className="relative h-[450px] md:h-[600px]">
          ...
          <div className="absolute bottom-12 right-8 left-8 md:right-16 md:left-16">
              ...
              <h1 className={`text-3xl md:text-5xl font-black leading-[1.2] tracking-tight ${darkMode ? 'text-white' : 'text-slate-900'}`}>{article.name}</h1>
          </div>
        </header>
        ...
        <div className="px-8 md:px-20 py-16">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-16 border-b border-slate-100 pb-12">
             ...
             <div className="flex flex-wrap items-center gap-3">
               <span className="text-slate-400 text-xs font-black ml-2 hidden sm:inline italic">شارك المقال:</span>
               
               <button onClick={handleNativeShare} className="p-3 bg-indigo-600 text-white rounded-xl shadow-lg hover:scale-110 active:scale-95 transition-all cursor-pointer" title="مشاركة عبر الجهاز">
                 <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                   <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" />
                 </svg>
               </button>

               <button onClick={shareOnWhatsApp} className="p-3 bg-[#25D366] text-white rounded-xl shadow-lg hover:scale-110 active:scale-95 transition-all cursor-pointer" title="واتساب">
               ...
               <button onClick={shareOnFacebook} className="p-3 bg-[#1877F2] text-white rounded-xl shadow-lg hover:scale-110 active:scale-95 transition-all cursor-pointer" title="فيسبوك">
                 <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
               </button>

               <button onClick={shareOnTwitter} className="p-3 bg-black text-white rounded-xl shadow-lg hover:scale-110 active:scale-95 transition-all cursor-pointer" title="إكس">
                 <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
               </button>

               <button onClick={handleCopyLink} className={`flex items-center gap-2 p-3 rounded-xl transition-all shadow-md cursor-pointer ${copyStatus ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-600'}`}>
                 {copyStatus ? <span className="text-[10px] font-black px-2">تم النسخ!</span> : <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3" /></svg>}
               </button>
             </div>
          </div>

          <div className={`prose prose-slate prose-xl max-w-none ${darkMode ? 'prose-invert' : ''}`}>{renderContent(article.content)}</div>

          {relatedArticles.length > 0 && (
            <div className="mt-24 pt-16 border-t border-slate-100">
              <h3 className={`text-2xl font-black mb-10 ${darkMode ? 'text-white' : 'text-slate-900'}`}>اقرأ المزيد في {article.category} 📚</h3>
          ...
          <div className="mt-24 border-t border-slate-100 pt-16">
            <h3 className={`text-2xl font-black mb-10 ${darkMode ? 'text-white' : 'text-slate-900'}`}>نقاشات القراء ({article.comments?.length || 0})</h3>
