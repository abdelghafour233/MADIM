...
const ArticleDetail: React.FC<ArticleDetailProps> = ({ article, onBack, siteName, adsenseCode, relatedArticles, onArticleClick, onLike, onAddComment, darkMode }) => {
  ...
  const handleCopyLink = async () => {
    try {
      if (navigator.clipboard && window.isSecureContext) {
        ...
        // fallback لنسخ الرابط في المتصفحات القديمة أو غير الآمنة
  ...
  const shareOnWhatsApp = () => {
    const text = encodeURIComponent(`شاهد هذا المقال المهم: ${article.name}\n${shareUrl}`);
  ...
  const shareOnTwitter = () => {
    const text = encodeURIComponent(`مقالة مميزة: ${article.name}`);
        ...
        <div className="px-8 md:px-20 py-16">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-16 border-b border-slate-100 pb-12">
             ...
             <div className="flex flex-wrap items-center gap-3">
               ...
               <button onClick={shareOnWhatsApp} className="p-3 bg-[#25D366] text-white rounded-xl shadow-lg hover:scale-110 active:scale-95 transition-all" title="واتساب">
               ...
               <button onClick={shareOnFacebook} className="p-3 bg-[#1877F2] text-white rounded-xl shadow-lg hover:scale-110 active:scale-95 transition-all" title="فيسبوك">
                 <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
               </button>

               <button onClick={shareOnTwitter} className="p-3 bg-black text-white rounded-xl shadow-lg hover:scale-110 active:scale-95 transition-all" title="إكس">
                 <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
               </button>

               <button onClick={handleCopyLink} className={`flex items-center gap-2 p-3 rounded-xl transition-all shadow-md ${copyStatus ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-600'}`}>
