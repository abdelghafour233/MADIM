...
const ArticleDetail: React.FC<ArticleDetailProps> = ({ article, onBack, siteName, adsenseCode, relatedArticles, onArticleClick, onLike, onAddComment, darkMode }) => {
  ...
  const shareOnWhatsApp = () => {
    const text = encodeURIComponent(`اقرأ هذا المقال الرائع: ${article.name}\n${shareUrl}`);
  ...
  const shareOnTwitter = () => {
    const text = encodeURIComponent(`مقالة مميزة من ${siteName}: ${article.name}`);
        ...
        <div className="px-8 md:px-20 py-16">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-16 border-b border-slate-100 pb-12">
             ...
             <div className="flex flex-wrap items-center gap-3">
               <span className="text-slate-400 text-xs font-black ml-2 hidden sm:inline">أنشر الفائدة:</span>
               
               {/* WhatsApp */}
               <button onClick={shareOnWhatsApp} className="p-3 bg-[#25D366] text-white rounded-xl shadow-lg hover:scale-110 active:scale-95 transition-all" title="شارك عبر واتساب">
               ...
               {/* Facebook */}
               <button onClick={shareOnFacebook} className="p-3 bg-[#1877F2] text-white rounded-xl shadow-lg hover:scale-110 active:scale-95 transition-all" title="شارك عبر فيسبوك">
                 <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
               </button>

               {/* Twitter / X */}
               <button onClick={shareOnTwitter} className="p-3 bg-black text-white rounded-xl shadow-lg hover:scale-110 active:scale-95 transition-all" title="شارك عبر إكس">
               ...
               <button onClick={handleCopyLink} className={`flex items-center gap-2 p-3 rounded-xl transition-all shadow-md ${copyStatus ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-600'}`}>
                 {copyStatus ? <span className="text-[10px] font-black">تم!</span> : <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3" /></svg>}
               </button>
             </div>
          </div>
