...
const ArticleDetail: React.FC<ArticleDetailProps> = ({ article, onBack, siteName, adsenseCode, relatedArticles, onArticleClick, onLike, onAddComment, darkMode }) => {
  ...
  // Extract Publisher ID from settings code
  ...
  const handleCommentSubmit = (e: React.FormEvent) => {
    ...
    setCommentName('');
    setCommentText('');
    alert('شكراً لمشاركتك! تم إضافة تعليقك.');
  };
    ...
    return paragraphs.map((para, i) => {
      ...
      return (
        <React.Fragment key={i}>
              ...
              if (part.match(urlRegex)) {
                return (
                  <div key={index} className="my-12 flex justify-center">
                    <a href={part} target="_blank" rel="noopener noreferrer" className="bg-emerald-600 hover:bg-emerald-700 text-white px-12 py-5 rounded-2xl font-black text-xl shadow-xl transition-all hover:scale-105 flex items-center gap-4">
                      <span>المصدر / المزيد من المعلومات 🔗</span>
                    </a>
                  </div>
                );
              }
          ...
          {/* Inject Ad Unit after specific paragraphs */}
      ...
      <div className={`rounded-[60px] shadow-2xl overflow-hidden border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-white'}`}>
        <header className="relative h-[450px] md:h-[600px]">
          ...
          <div className="absolute bottom-12 right-8 left-8 md:right-16 md:left-16">
              <span className="bg-emerald-600 text-white text-xs font-black px-4 py-2 rounded-lg mb-4 inline-block shadow-lg">
                {article.category}
              </span>
        ...
        <div className="px-8 md:px-20 py-16">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-16 border-b border-slate-50/10 pb-12">
             <div className="flex items-center gap-6">
               <button 
                onClick={handleLike}
                className={`flex items-center gap-3 px-8 py-4 rounded-2xl font-black text-sm transition-all ${liked ? 'bg-red-500 text-white shadow-lg shadow-red-500/20' : (darkMode ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-600')}`}
               >
             ...
             <div className="flex flex-wrap items-center gap-3">
               <span className="text-xs font-black text-slate-400 uppercase tracking-widest ml-2">مشاركة:</span>
               
               {/* Native Share Button (Mobile) */}
               {typeof navigator !== 'undefined' && navigator.share && (
                 <button onClick={handleNativeShare} className="p-4 bg-slate-900 text-white rounded-2xl hover:scale-110 transition-all shadow-md">
               ...
               <a 
                ...
                className="p-4 bg-[#1877F2] text-white rounded-2xl hover:scale-110 transition-all shadow-md"
               >
               ...
               {/* X (Twitter) */}
               <a 
                href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(article.name)}&url=${encodeURIComponent(shareUrl)}`} 
                target="_blank" 
                rel="noopener noreferrer" 
                className="p-4 bg-black text-white rounded-2xl hover:scale-110 transition-all shadow-md"
               >
               ...
               {/* WhatsApp */}
               <a 
                href={`https://wa.me/?text=${encodeURIComponent(article.name)}%20${encodeURIComponent(shareUrl)}`} 
                target="_blank" 
                rel="noopener noreferrer" 
                className="p-4 bg-[#25D366] text-white rounded-2xl hover:scale-110 transition-all shadow-md"
               >
               ...
               <button 
                ...
                className={`flex items-center gap-2 p-4 rounded-2xl transition-all shadow-md ${copyStatus ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-600'}`}
                title="نسخ رابط المقال"
               >
                 {copyStatus ? (
                   <>
                    <span className="text-xs font-black">تم النسخ!</span>
          ...
          <div className="my-16">
            <AdUnit publisherId={publisherId} format="rectangle" />
          </div>
          ...
          <div className="mt-24 border-t border-slate-100 pt-16">
            ...
            <form onSubmit={handleCommentSubmit} className="mb-16 space-y-6">
              <input 
                className={`w-full p-5 rounded-2xl outline-none font-bold ${darkMode ? 'bg-slate-800 text-white border-slate-700' : 'bg-slate-50 text-slate-900 border-transparent'} border-2 focus:border-emerald-500 transition-all`}
              ...
              />
              <textarea 
                className={`w-full p-5 rounded-3xl outline-none font-medium h-32 ${darkMode ? 'bg-slate-800 text-white border-slate-700' : 'bg-slate-50 text-slate-900 border-transparent'} border-2 focus:border-emerald-500 transition-all`}
                placeholder="ما رأيك في هذا الموضوع؟ شاركنا أفكارك..."
                value={commentText}
                onChange={e => setCommentText(e.target.value)}
                required
              />
              <button type="submit" className="bg-emerald-600 text-white px-12 py-4 rounded-2xl font-black hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-500/20">نشر التعليق</button>
            </form>
            ...
            <div className="space-y-8">
              {article.comments?.map(c => (
                <div key={c.id} className={`p-8 rounded-[32px] ${darkMode ? 'bg-slate-800/50 border border-slate-800' : 'bg-white border border-slate-100 shadow-sm'}`}>
                  <div className="flex justify-between items-center mb-4">
                    <span className="font-black text-emerald-500 flex items-center gap-2">
              ...
              {(!article.comments || article.comments.length === 0) && (
                <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-[40px]">
                   <p className="text-slate-400 font-bold">لا توجد تعليقات بعد. كن أول من يفتح باب النقاش!</p>
                </div>
              )}
            </div>
          </div>
